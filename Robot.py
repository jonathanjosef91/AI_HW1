...
class BreadthFirstSearchRobot(Robot):
    ...
    def solve(self, maze: MazeProblem, time_limit=float("inf")):
        ...
        while True:
            ...
            for new_state, new_cost in maze.expand_state(next_node.state):
                if new_state not in self.close and new_state not in self.queue:
                    new_node = Node(new_state, parent=next_node, g_value=next_node.g_value + new_cost)
                    if maze.is_goal(new_state):
        ...
        while True:
            ...
            for new_state, new_cost in maze_problem.expand_state(next_node.state):
                ...
                if new_state in self.close:
                    old_node = self.close.get_node(new_state)
                    if self._calc_node_priority(old_node) > self._calc_node_priority(new_node):
